...
def evaluate(model, device, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    ...
    for input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx in eval_dataloader:
        ...
        with torch.no_grad():
            logits, rep = model(input_ids, segment_ids, input_mask, labels=None, sub_idx=sub_idx, obj_idx=obj_idx)
        ...
        if len(preds) == 0:
            ...
            reps[0] = np.append(
                    reps[0], rep.detach().cpu().numpy(), axis=0)

    eval_loss = eval_loss / nb_eval_steps
    reps = reps[0]
    #reps_dict = {k:reps[k] for k in range(len(reps))}
    #labels_dict = {k:eval_label_ids[k] for k in range(len(eval_label_ids))}
    #with open("train_datastore.json", "w") as f:
    #    json.dump(reps_dict, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc', 'wiki80','i2b2','tacred'])
