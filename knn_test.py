...
from knn_faiss import knn_faiss
...
def save_evaluate(task, test_type,model, device, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    model.eval()
    eval_loss = 0
    nb_eval_steps = 0
    datastore = []
    ...
    for input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx in eval_dataloader:
        ...
        #if len(eval_label_ids) == 0:
        #    eval_label_ids.append(label_ids.numpy())
        #else:
        #    eval_label_ids[0] = np.append(eval_label_ids[0], label_ids.numpy())
        label_ids = label_ids.to(device)
        sub_idx = sub_idx.to(device)
        obj_idx = obj_idx.to(device)
        with torch.no_grad():
            logits, tr_rep = model(input_ids, segment_ids, input_mask, labels=None, sub_idx=sub_idx, obj_idx=obj_idx)
        if len(datastore) == 0:
            datastore.append(tr_rep.detach().cpu().numpy())
        else:
            datastore[0] = np.append(
                datastore[0], tr_rep.detach().cpu().numpy(), axis=0)
        if len(preds) == 0:
    ...
    knn_datastore = datastore[0]
    logits_distribution = preds[0]

    preds2 = np.argmax(preds[0], axis=1)
    output_datastore = "datastore/{}/{}_{}_datastore".format(task,task,test_type)
    if not os.path.exists(output_datastore):
        os.mkdir(output_datastore)
    np.savetxt("datastore/{}/{}_{}_datastore/datastore.csv".format(task,task,test_type), knn_datastore, delimiter=",")
    np.savetxt("datastore/{}/{}_{}_datastore/label.csv".format(task,task,test_type), eval_label_ids, delimiter=",")
    np.savetxt("datastore/{}/{}_{}_datastore/distribution.csv".format(task,task,test_type), logits_distribution, delimiter=",")
    
    result = compute_f1(preds2, eval_label_ids, e2e_ngold=e2e_ngold)

    #result['accuracy'] = simple_accuracy(preds2, eval_label_ids)
    if verbose:
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
    return preds2, result, logits


    #preds1, preds2, preds3, preds4, preds5 = knn_bert(bert_distribution, test_datastore, label_dict, embeddings, 4, 0.2)
    #preds = knn_re(knn_datastore, train_label_ids, test_datastore, bert_distribution, 4, 0.2)
#def knn_re(datastore, train_labels, test_reps, bert_preds, k, ratio):

def knn_test(model, device, train_dataloader, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    model.eval()
    eval_loss = 0
    nb_eval_steps = 0
    preds = []
    ...
    for input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx in eval_dataloader:
        ...
        obj_idx = obj_idx.to(device)
        with torch.no_grad():
            logits, eval_rep = model(input_ids, segment_ids, input_mask, labels=None, sub_idx=sub_idx, obj_idx=obj_idx)
        ...
        if len(preds) == 0:
            preds.append(logits.detach().cpu().numpy())
            eval_reps.append(eval_rep.detach().cpu().numpy())
        else:
            preds[0] = np.append(
                preds[0], logits.detach().cpu().numpy(), axis=0)
            eval_reps[0] = np.append(
                eval_reps[0], eval_rep.detach().cpu().numpy(), axis=0)


    eval_loss = eval_loss / nb_eval_steps
    knn_datastore = np.loadtxt("ace_train_datastore/knn_datastore", delimiter=",")
    train_label_ids = np.loadtxt("ace_train_datastore/knn_label", delimiter=",")
    train_logits = np.loadtxt("ace_train_datastore/knn_logits", delimiter=",")
    test_datastore = eval_reps[0]
    
    print(len(train_label_ids))
    print(len(test_datastore))
    #label_dict = {k:train_label_ids[0][k] for k in range(len(train_label_ids))}
    #embeddings = {k:test_datastore[k] for k in range(len(test_datastore))}
    bert_distribution = softmax(preds[0], axis=1)
    #preds1, preds2, preds3, preds4, preds5 = knn_bert(bert_distribution, test_datastore, label_dict, embeddings, 4, 0.2)
    preds = knn_faiss(knn_datastore, train_label_ids, train_logits, test_datastore,bert_distribution, 4, 0.2)
    
    #eval_dict["labels"] = eval_label_ids
    #preds = knn_re(knn_datastore, train_label_ids, test_datastore, bert_distribution, 4, 0.2)
...
def knn_evaluate(model, device, train_dataloader, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    model.eval()
    eval_loss = 0
    nb_eval_steps = 0
    datastore = []
    ...
    train_label_ids = []
    tr_preds = []
    for input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx in train_dataloader:
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        if len(train_label_ids) == 0:
            train_label_ids.append(label_ids.numpy())
        else:
            train_label_ids[0] = np.append(train_label_ids[0], label_ids.numpy())
        label_ids = label_ids.to(device)
        sub_idx = sub_idx.to(device)
        obj_idx = obj_idx.to(device)
        with torch.no_grad():
    ...
    knn_datastore = datastore[0]
    test_datastore = eval_reps[0]
    print(len(train_label_ids[0]))
    print(len(test_datastore))
    #label_dict = {k:train_label_ids[0][k] for k in range(len(train_label_ids[0]))}
    #embeddings = {k:test_datastore[k] for k in range(len(test_datastore))}
    tr_distribution = softmax(tr_preds[0], axis=1)
    bert_distribution = softmax(preds[0], axis=1)
    #preds1, preds2, preds3, preds4, preds5 = knn_bert(bert_distribution, test_datastore, label_dict, embeddings, 4, 0.2)
    preds2 = knn_faiss(knn_datastore, train_label_ids[0],tr_distribution, test_datastore, bert_distribution, 4, 0.7)
    
    #eval_dict["labels"] = eval_label_ids
    #preds = knn_re(knn_datastore, train_label_ids, test_datastore, bert_distribution, 4, 0.2)
    result = compute_f1(preds2, eval_label_ids.numpy(), e2e_ngold=e2e_ngold)
    result['accuracy'] = simple_accuracy(preds2, eval_label_ids.numpy())
    result['eval_loss'] = eval_loss
    if verbose:
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
...
def main(args):
    ...
    # train set
    if args.do_train:
        train_dataset, train_examples, train_nrel = generate_relation_data(args.task,args.train_file, use_gold=True, context_window=args.context_window)
    # dev set
    if (args.do_eval and args.do_train) or (args.do_eval and not(args.eval_test)):
        eval_dataset, eval_examples, eval_nrel = generate_relation_data(args.task,os.path.join(args.entity_output_dir, args.entity_predictions_dev), use_gold=args.eval_with_gold, context_window=args.context_window)
    # test set
    if args.eval_test:
        test_dataset, test_examples, test_nrel = generate_relation_data(args.task,os.path.join(args.entity_output_dir, args.entity_predictions_test), use_gold=args.eval_with_gold, context_window=args.context_window)
    ...
    if args.do_eval:
        ...
        #train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_sub_idx, all_obj_idx)
        #train_dataloader = DataLoader(train_data, batch_size=args.train_batch_size)
        #3train_batches = [batch for batch in train_dataloader]
        ...
        model.to(device)
        preds, result, logits = save_evaluate(args.task, args.test_type, model, device, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=eval_nrel)
        #preds, result, logits = knn_test(model, device, train_dataloader, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=eval_nrel)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc','i2b2','tacred','wiki80'])
    parser.add_argument('--test_type', type=str, default=None, required=True, choices=['train', 'dev', 'test'])
